...
export default function PropwatchPage() {
  ...
  function toggleAlert(key: string, name: string) {
    ...
    showToast(newState ? `Alerts on for ${name}` : 'Alerts disabled');
  }
  ...
  function searchAnyFirm() {
    if (!search) { showToast('Type a firm name first'); return; }
    if (!isLoggedIn) { setShowModal(true); return; }
    router.push('/monitor?prompt=' + encodeURIComponent('Monitor my ' + search + ' account'));
  }

  const riskColor = (risk: number) =>
    risk < 35 ? 'var(--safe)' : risk < 60 ? 'var(--warn)' : 'var(--danger)';

  if (!authChecked) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 40, marginBottom: 12, animation: 'shieldPulse 1.5s infinite' }}>🛡️</div>
          <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 18, color: 'var(--ink-muted)' }}>Loading PropWatch...</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      {showModal && <PremiumModal onClose={() => {
        if (!isLoggedIn) router.push('/login');
        else setShowModal(false);
      }} />}

      <div className="propwatch-layout" style={{ filter: !isLoggedIn ? 'blur(3px)' : 'none', pointerEvents: !isLoggedIn ? 'none' : 'auto', userSelect: !isLoggedIn ? 'none' : 'auto', transition: 'filter 0.3s ease' }}>
        <div className="propwatch-header">
          ...
          <div className="propwatch-search-wrap">
            <div className="propwatch-search-box">
              ...
              />
              {search && <button className="propwatch-search-clear" onClick={() => setSearch('')}>X</button>}
            </div>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '60px 24px' }}>
            ...
            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: 22, fontWeight: 700, marginBottom: 8 }}>No firms match {search}</div>
            <p style={{ fontSize: 14, color: 'var(--ink-muted)', marginBottom: 20 }}>You can still monitor any firm in the world.</p>
            <button className="btn btn-accent" onClick={searchAnyFirm}>🌍 Monitor {search} →</button>
          </div>
        ) : (
      ...
      <footer>
        ...
        <div>PropWatch — {PROP_FIRMS.length} firms monitored</div>
