...
useEffect(() => {
  ...
  const checkNow = useCallback(async (id: string) => {
    ...
    try {
      ...
      try {
        ...
        if (r.ok) { text = (await r.text()).replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim(); }
      } catch {}
      const newHash = simpleHash(text.slice(0, 3000));
      const updated = monitors.map(mon => {
        if (mon.id !== id) return mon;
        if (!mon.snapshotHash) { showToast('✓ Baseline saved for ' + mon.firmName); return { ...mon, snapshotHash: newHash, status: 'monitoring' as const, lastChecked: new Date().toISOString() }; }
        if (mon.snapshotHash !== newHash) { showToast('⚠️ Change at ' + mon.firmName + '!'); return { ...mon, snapshotHash: newHash, status: 'changed' as const, lastChecked: new Date().toISOString(), changeLog: [{ detectedAt: new Date().toISOString(), summary: 'Change detected.', impact: '' }, ...mon.changeLog] }; }
  ...
  function saveUrl(id: string) {
    if (editUrl && !editUrl.startsWith('http')) { showToast('URL must start with https://'); return; }
    ...
    <>
      <div className="monitor-layout">
        <div className="monitor-page-header">
          ...
          <p>Tell ClauseGuard which specific account you are on. We track that page and show you what changed — and what it means for your money.</p>
        </div>
        ...
        <div className="monitor-add-card">
          ...
          <div className="monitor-add-body">
            <textarea
              ...
              placeholder={'"Monitor my Blue Guardian Instant Starter $5000 account"\n"Watch FTMO Standard Challenge 10k"\n"Track Apex Trader futures funded account"'}
              rows={3}
            />
            <div className="monitor-url-field">
              <label className="monitor-url-label">T&C URL (optional — AI will guess if you skip)</label>
              <input type="text" className="url-input" style={{ width: '100%', marginTop: 6 }} value={urlInput} onChange={e => setUrlInput(e.target.value)} placeholder="https://example.com/terms" />
            </div>
            <div className="monitor-examples" style={{ marginTop: 10 }}>
              <span className="monitor-example-label">Try:</span>
              {['Monitor my Blue Guardian Instant Starter $5000 account', 'Watch FTMO Standard Challenge $10,000', 'Track Apex Trader futures funded account'].map((t, i) => (
                <button key={i} className="quick-pick" onClick={() => setPrompt(t)}>{['Blue Guardian', 'FTMO 10k', 'Apex Futures'][i]}</button>
              ))}
            </div>
            <button className="scan-btn" style={{ width: '100%', justifyContent: 'center', marginTop: 12 }} onClick={handleAdd} disabled={adding}>
        ...
        {monitors.length === 0 ? (
          ...
          <div className="monitor-list">
            {monitors.map(m => {
              const dot = m.status === 'changed' ? 'status-changed' : m.status === 'pending' ? 'status-pending' : 'status-monitoring';
              const badge = m.status === 'changed' ? <span className="monitor-badge-new">⚠ Changed</span> : <span className="monitor-badge-ok">✓ OK</span>;
              return (
                <div key={m.id} className="monitor-card" onClick={() => { setSelectedMonitor(m); setEditUrl(m.tcUrl || ''); }}>
                  <div className="monitor-card-top">
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
                      ...
                      <div><div className="monitor-card-firm">{m.firmName}</div><div className="monitor-card-account">{m.accountType}{m.accountSize !== 'Not specified' ? ' · ' + m.accountSize : ''}</div></div>
                    </div>
                    {badge}
                  </div>
                       <div className="monitor-card-meta">
                    <div className="monitor-meta-item">🗓 <strong>{new Date(m.addedAt).toLocaleDateString('en-ZA', { day: 'numeric', month: 'short' })}</strong></div>
                    {m.lastChecked && <div className="monitor-meta-item">🔄 <strong>{timeAgo(m.lastChecked)}</strong></div>}
                  </div>
                  {m.tcUrl ? <div className="monitor-card-url">🔗 {m.tcUrl}</div> : <div className="monitor-card-url" style={{ color: 'var(--ink-muted)' }}>No URL — tap to add</div>}
                  <div className="monitor-card-footer">
                    <span className="monitor-card-date">{m.description || m.firmName}</span>
          ...
          <div className="modal-card modal-card-wide" onClick={e => e.stopPropagation()}>
              ...
              <div className="parsed-result" style={{ marginBottom: 16 }}>
                ...
                <div className="parsed-row" style={{ flexDirection: 'column', gap: 6 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <input type="text" className="url-input" value={editUrl} onChange={e => setEditUrl(e.target.value)} style={{ flex: 1, fontSize: 11 }} onClick={e => e.stopPropagation()} />
                    <button className="btn btn-accent" style={{ padding: '8px 14px', fontSize: 12 }} onClick={e => { e.stopPropagation(); saveUrl(selectedMonitor.id); }}>💾 Save</button>
                  </div>
                </div>
              </div>
            </div>
            <div className="change-log">
              {selectedMonitor.changeLog.length > 0
                ...
                : <div className="no-changes">✅ No changes yet. Click &ldquo;Check Now&rdquo; below.</div>}
            </div>
            <div style={{ padding: '16px 24px', borderTop: '1px solid var(--border)', display: 'flex', gap: 10, flexWrap: 'wrap' }}>
              {selectedMonitor.tcUrl
                ? <button className="btn btn-accent" style={{ flex: 1, justifyContent: 'center' }} disabled={checkingId === selectedMonitor.id} onClick={() => checkNow(selectedMonitor.id)}>
                    {checkingId === selectedMonitor.id ? '⏳ Checking...' : '🔄 Check for Changes Now'}
                  </button>
                : <button className="btn btn-ghost" style={{ flex: 1, justifyContent: 'center' }}>Add URL to enable checking</button>}
...
export default function MonitorPage() {
  return (
    <Suspense fallback={<div style={{ padding: '100px 40px', fontFamily: 'Cormorant Garamond, serif', fontSize: 20 }}>Loading...</div>}>
      <MonitorPageInner />
    </Suspense>
  );
}